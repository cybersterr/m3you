...
// ================= (REST UNTOUCHED) =================
function convertJioJson(json){ const out=[]; for(const id in json){ const ch=json[id]; const cookie=`hdnea=${ch.url.match(/__hdnea__=([^&]*)/)?.[1]||""}`; const category=(ch.group_title||"GENERAL").toUpperCase(); out.push(`#EXTINF:-1 tvg-id="${id}" tvg-logo="${ch.tvg_logo}" group-title="JIOTV+ | ${category}",${ch.channel_name}`); out.push(`#KODIPROP:inputstream.adaptive.license_type=clearkey`); out.push(`#KODIPROP:inputstream.adaptive.license_key=${ch.kid}:${ch.key}`); out.push(`#EXTHTTP:${JSON.stringify({Cookie:cookie,"User-Agent":ch.user_agent})}`); out.push(ch.url);} return out.join("\n");}

function convertSony(json){ if(!json.matches) return ""; return json.matches.filter(m=>m.isLive).map(m=>{ const url=m.dai_url||m.pub_url; if(!url) return null; return `#EXTINF:-1 tvg-logo="${m.src}" group-title="SonyLiv | Sports",${m.match_name}\n${url}`; }).filter(Boolean).join("\n");}

function convertSonyJsonChannels(json){ if(!json || typeof json !== "object") return ""; const out=[]; for(const id in json){ const ch = json[id]; if(!ch.url) continue; out.push(`#EXTINF:-1 tvg-id="${id}" tvg-logo="${ch.tvg_logo || ""}" group-title="CS OTT | SONY LIV",${ch.channel_name || id}`); out.push(ch.url);} return out.join("\n");}

function convertSunxtJson(json){ if(!Array.isArray(json)) return ""; const out=[]; json.slice(1).forEach((ch, i)=>{ if(!ch.mpd_url) return; out.push(`#EXTINF:-1 tvg-id="${ch.id || 3000+i}" tvg-logo="${ch.logo || ""}" group-title="CS OTT | SUNXT",${ch.name || "SunXT Channel"}`); out.push(`#KODIPROP:inputstream.adaptive.license_type=clearkey`); out.push(`#KODIPROP:inputstream.adaptive.license_key=${(ch.license_url || "").split("keyid=")[1]?.split("&")[0] || ""}:${(ch.license_url || "").split("key=")[1] || ""}`); out.push(`#EXTHTTP:${JSON.stringify({"User-Agent": ch.user_agent || ""})}`); out.push(ch.mpd_url); }); return out.join("\n");}

function convertSportsJson(json){ if(!json || !Array.isArray(json.streams)) return ""; const out=[]; json.streams.forEach((s,i)=>{ if(!s.url) return; const urlObj=new URL(s.url); const drm=urlObj.searchParams.get("drmLicense")||""; const[kid,key]=drm.split(":"); const ua=urlObj.searchParams.get("User-Agent")||""; const hdnea=urlObj.searchParams.get("__hdnea__")||""; urlObj.searchParams.delete("drmLicense"); urlObj.searchParams.delete("User-Agent"); out.push(`#EXTINF:-1 tvg-id="${1100+i}" tvg-logo="https://i.ibb.co/9HfRQcP2/unnamed-removebg-preview.png" group-title="IPL LIVE",${s.language || "IPL Live"}`); out.push(`#KODIPROP:inputstream.adaptive.license_type=clearkey`); out.push(`#KODIPROP:inputstream.adaptive.license_key=${kid}:${key}`); out.push(`#EXTHTTP:${JSON.stringify({Cookie:hdnea?`__hdnea__=${hdnea}`:"","User-Agent":ua})}`); out.push(urlObj.toString());}); return out.join("\n");}

async function safeFetch(url){ try{ const res=await axios.get(url,{timeout:60000}); return res.data;}catch{return null;}}

function extractObjects(obj, arr = []) { if (Array.isArray(obj)) { obj.forEach(o => extractObjects(o, arr)); } else if (obj && typeof obj === "object") { arr.push(obj); Object.values(obj).forEach(v => extractObjects(v, arr)); } return arr;}
...
async function run(){
 ...
 if(newm3u){
  ...
  out.push(section("CS-W | Extra"), lines.join("\n"));
 }
